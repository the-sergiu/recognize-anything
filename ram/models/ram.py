...
class RAM(nn.Module):
    ...
    def generate_tags_for_batch_v2(
        self,
        images,  # images is now a list or tensor of images
        threshold=0.68,
        tag_input=None,
        ...
        ).repeat(images.shape[0], 1, 1)
        image_embeds = self.image_proj(self.visual_encoder(images))
        image_atts = torch.ones(
            image_embeds.size()[:-1], dtype=torch.int32, device=device
        )
        ...
        # Zero out the delete_tag_index
        targets[:, self.delete_tag_index] = 0

        tag_output = []
        for b in range(targets.size(0)):
            index = (targets[b] == 1).nonzero(as_tuple=True)[0].cpu()
